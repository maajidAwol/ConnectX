...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        elif self.action == 'list' or self.action in ['sales', 'listings', 'products', 'my_orders', 'by_customer']:
    ...
    def get_queryset(self):
        ...
        if tenant_owner_permission.has_permission(self.request, self):
            ...
            return queryset.filter(
                ...
                Q(items__product_owner=tenant)  # Orders with products owned by this tenant
            ).distinct()
    ...
    def perform_create(self, serializer):
        ...
        today_orders_count = Order.objects.filter(created_at__gte=today_start).count() + 1
        ...
        if selling_tenant_id:
            ...
            try:
                ...
                selling_tenant = user.tenant  # Fallback to user's tenant if ID is invalid
        else:
    ...
    def tenant_statistics(self, request):
        ...
        if start_date:
            try:
                start_datetime = datetime.datetime.strptime(start_date, '%Y-%m-%d').replace(tzinfo=timezone.utc)
                all_orders_queryset = all_orders_queryset.filter(created_at__gte=start_datetime)
            except ValueError:
                pass
        ...
        if end_date:
            try:
                end_datetime = datetime.datetime.strptime(end_date, '%Y-%m-%d').replace(hour=23, minute=59, second=59, tzinfo=timezone.utc)
                all_orders_queryset = all_orders_queryset.filter(created_at__lte=end_datetime)
            except ValueError:
                pass
        ...
        total_product_orders = all_orders_queryset.filter(items__product_owner=tenant).distinct().count()
        
        # Get counts by status for orders where tenant is seller
        status_counts = {}
        for status_choice in Order.STATUS_CHOICES:
            status_code = status_choice[0]
            status_counts[status_code] = all_orders_queryset.filter(tenant=tenant, status=status_code).count()
        
        # Get stats on product orders
        # Count of orders with products owned and sold by tenant
        owned_and_sold = all_orders_queryset.filter(
            tenant=tenant,
            items__product_owner=tenant
        ).distinct().count()
        
        # Count of orders with products owned by tenant but sold by others
        owned_not_sold = all_orders_queryset.filter(
            ~Q(tenant=tenant),
            items__product_owner=tenant
        ).distinct().count()
        
        # Count of orders with products not owned by tenant but sold by tenant
        sold_not_owned = all_orders_queryset.filter(
            tenant=tenant
        ).filter(~Q(items__product_owner=tenant)).distinct().count()
        
        # Calculate revenue from different sources
        from django.db.models import Sum
        
        # Revenue from products owned and sold by tenant
        revenue_owned_products = all_orders_queryset.filter(
            tenant=tenant,
            items__product_owner=tenant
        ).aggregate(total=Sum('total_amount'))['total'] or 0
        
        # Revenue from reselling other tenants' products
        revenue_resold_products = all_orders_queryset.filter(
            tenant=tenant
        ).filter(~Q(items__product_owner=tenant)).aggregate(total=Sum('total_amount'))['total'] or 0
    ...
    def product_statistics(self, request):
        ...
        if start_date:
            try:
                start_datetime = datetime.datetime.strptime(start_date, '%Y-%m-%d').replace(tzinfo=timezone.utc)
                order_items_queryset = order_items_queryset.filter(order__created_at__gte=start_datetime)
            except ValueError:
                pass
        ...
        if end_date:
            try:
                end_datetime = datetime.datetime.strptime(end_date, '%Y-%m-%d').replace(hour=23, minute=59, second=59, tzinfo=timezone.utc)
                order_items_queryset = order_items_queryset.filter(order__created_at__lte=end_datetime)
            except ValueError:
                pass
        
        # Total sales stats
        total_quantity_sold = order_items_queryset.aggregate(Sum('quantity'))['quantity__sum'] or 0
        total_sales_count = order_items_queryset.count()
        
        # Get stats by seller
        sales_by_seller = order_items_queryset.values('order__tenant__name').annotate(
            order_count=Count('order', distinct=True),
            quantity_sold=Sum('quantity')
        ).order_by('-quantity_sold')
        
        # Get stats by status
        sales_by_status = order_items_queryset.values('order__status').annotate(
            order_count=Count('order', distinct=True),
            quantity_sold=Sum('quantity')
