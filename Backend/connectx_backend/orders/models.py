...
class OrderProductItem(models.Model):
    ...
    custom_profit_percentage = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
    custom_selling_price = models.DecimalField(max_digits=8, decimal_places=2, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.custom_profit_percentage is not None:
            self.custom_selling_price = self.product.base_price * (1 + self.custom_profit_percentage / 100)
        else:
