  ...
  const handleSave = async () => {
    try {
      ...
      const textFields = {
        ...
        business_registration_number: formData.get('business_registration_number') || tenantData?.business_registration_number,
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          {isEditing ? (
            ...
            <Button variant="default" onClick={() => setIsEditing(true)} className="gap-2" disabled={isLoading}>
                  ...
                  <>
                    ...
                    <Building className="h-4 w-4 text-muted-foreground" />
                      ...
                      {tenantData?.tenant_verification_status ? (
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${tenantData.tenant_verification_status === 'verified' ? 'bg-green-100 text-green-800' : 'bg-gray-700 text-gray-200'}`}>
                          {tenantData.tenant_verification_status.charAt(0).toUpperCase() + tenantData.tenant_verification_status.slice(1)}
                        </span>
                      ) : (
                        <span className="px-2 py-1 rounded text-xs font-semibold bg-gray-700 text-gray-200">Unknown</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mb-2">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span className="font-medium">Founded:</span>
                      <span>{tenantData?.licence_registration_date ? new Date(tenantData.licence_registration_date).getFullYear() : '----'}</span>
                    </div>
                  </>
                )}
                {isLoading ? (
